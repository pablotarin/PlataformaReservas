  ...
  calendar = new FullCalendar.Calendar(calendarEl, {
    ...
    events: async (info, successCallback, failureCallback) => {
      try {
        ...
        const events = slots.map((s) => {
          ...
          // OJO: el estado real en BD para una cita confirmada es "reservado"
          // (la constraint de slots.status solo admite libre/pendiente/reservado).
    ...
    eventClick: async (clickInfo) => {
      ...
      if (slot.status === "libre") {
        const confirmDelete = await showConfirmModal(`¿Eliminar el hueco libre de las ${slot.time}?`, {
...
// ---------------------------------------------------------------------------
// COMPROBACIÓN DE CITAS PASADAS (solo visual, el backend es quien manda)
// ---------------------------------------------------------------------------
...
function openBookingModal(slot) {
  ...
  bookingStatusBadge.innerHTML =
    slot.status === "pendiente"
      ? '<span class="badge-pendiente">Pendiente de aceptar</span>'
      : '<span class="badge-reservado">Cita confirmada</span>';

  const pastMessage = past
    ? '<p class="past-booking-warning"><b>Esta cita ya ha pasado y no puede cancelarse.</b></p>'
    : "";

  bookingDetail.innerHTML = `
    <p><b>Fecha:</b> ${slot.date} a las ${slot.time}</p>
    <p><b>Cliente:</b> ${b ? escapeHtml(b.name) : "-"}</p>
    <p><b>Teléfono:</b> ${b ? escapeHtml(b.phone) : "-"}</p>
    <p><b>Email:</b> ${b ? escapeHtml(b.email) : "-"}</p>
    <p><b>Servicio:</b> ${b && b.serviceName ? escapeHtml(b.serviceName) : "-"}</p>
    <p><b>Notas:</b> ${b && b.notes ? escapeHtml(b.notes) : "-"}</p>
    ${pastMessage}
  `;
...
acceptBookingBtn.addEventListener("click", async () => {
  ...
  if (currentBookingStatus !== "pendiente") {
    showToast("Esta solicitud ya no está pendiente", "error");
    return;
  }
...
rejectBookingBtn.addEventListener("click", async () => {
  if (!currentBookingId || !currentBookingSlot) return;

  if (isSlotPast(currentBookingSlot)) {
    showToast("Esta cita ya ha pasado y no puede cancelarse", "error");
    return;
  }

  const msg =
    currentBookingStatus === "pendiente"
...
servicesTbody.addEventListener("click", async (e) => {
  ...
  if (action === "delete") {
    const confirmDelete = await showConfirmModal("¿Eliminar este servicio del listado de precios?", {
